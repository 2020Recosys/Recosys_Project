#!/usr/bin/env python
# coding: utf-8
# 전처리한 데이터에서 X, y memmap file 생성
# X, y memmap file에서 CV=10 split한 memmap file 차례대로 생성
...
cv = StratifiedKFold(10, shuffle=True, random_state=42).split(new_X_np, new_Y_np)
print('Made cv!')
...
for train_idx, test_idx in cv:
    ...
    # 파일기반 행렬 만들기 - X_train
    fn = 'mem_file_X_train_' + str(idx) + '.dat'
    ...
    # 파일기반 행렬 만들기 - X_test
    fn = 'mem_file_X_test_' + str(idx) + '.dat'
    mem_file_name = make_path(fn, directory='')
    ...
    # 파일기반 행렬 만들기 - y_train
    fn = 'mem_file_y_train_' + str(idx) + '.dat'
    mem_file_name = make_path(fn, directory='')
    ...
    # 파일기반 행렬 만들기 - y_test
    fn = 'mem_file_y_test_' + str(idx) + '.dat'
    mem_file_name = make_path(fn, directory='')
